...
class TeaGameNotifier extends StateNotifier<TeaGameState> {
  ...
  void generateItems() {
    ...
    state = state.copyWith(
      ...
      currentItems: items,
      currentItemNames: items.map((e) => e.name).toList(),
    );
  }

  void selectItem(String itemName) {
    final selectedItems = [...state.userSelectedItems, itemName];
    state = state.copyWith(userSelectedItems: selectedItems);
  }
  ...
  void checkItems() {
    ...
    if (correctItems.length != state.currentItemNames.length) {
      state = state.copyWith(
        message:
            "You missed some items ${correctItems.length} != ${state.currentItemNames.length}",
        backgroundColor: Colors.red.shade300,
      );
      return;
    }

    final isCorrect = correctItems
        .every((element) => state.currentItemNames.contains(element));
    if (isCorrect) {
      state = state.copyWith(
